...
function getSourceLines(lines: string[], startLine: number, endLine: number, context: number = 0) {
    ...
    for (let i = start; i <= end; i++) {
        ...
        const content = isMatch ? chalk.white(lineContent) : chalk.gray(lineContent);
...
async function main() {
    await yargs(hideBin(process.argv))
        .command("search <file> <query>", "Search AST", (yargs) => {
            return yargs
                .positional("file", { type: "string", demandOption: true })
                .positional("query", { type: "string", demandOption: true })
                .option("type", { alias: "t", type: "string", description: "Filter node type" })
                .option("context", { alias: "C", type: "number", default: 1, description: "Source lines context" })
                .option("limit", { alias: "l", type: "number", default: 10 })
                .option("declaration", { alias: "d", type: "boolean", description: "Find variable/function definition only" })
                .option("exact", { alias: "e", type: "boolean", description: "Exact match for identifiers/literals" });
        }, (argv) => runSearch(argv))
        .command("view <file> [range]", "View file content", (yargs) => {
            return yargs
                .positional("file", { type: "string", demandOption: true })
                .positional("range", { type: "string", description: "start:end", default: "1:50" });
        }, (argv) => runView(argv))
        .help()
        .strict()
        .parse();
...
async function runSearch(argv: any) {
    ...
    const ast = parser.parse(code, { sourceType: "module", plugins: [], tokens: false });
    ...
    traverse.default(ast, {
        enter(path: any) {
            ...
            if (declaration) {
                if (!t.isVariableDeclarator(node) && !t.isFunctionDeclaration(node) && !t.isClassDeclaration(node)) {
                    return;
                }
                // Check id
                if (!node.id || !t.isIdentifier(node.id)) return;
                if (exact ? node.id.name !== query : !node.id.name.includes(query)) return;
            }

            let matchFound = false;
            
            if (!declaration) {
                const check = (val: string) => exact ? val === query : val.includes(query);
                
                if (node.value && check(String(node.value))) matchFound = true;
                else if (node.name && check(String(node.name))) matchFound = true;
                else if (node.key && node.key.name && check(String(node.key.name))) matchFound = true;
                else if (node.quasis) {
            ...
            if (matchFound) {
                ...
                console.log(chalk.green(`Match #${matches} [${node.type}] at line ${loc.start.line}`));
                console.log(chalk.cyan("Path: ") + generateBreadcrumbs(path));
                
                // Show Source
                console.log(getSourceLines(lines, loc.start.line, loc.end.line, context));
            }
        }
    });
