...
function isSkillListingAttachment(path: NodePath<t.ObjectExpression>): boolean {
    ...
    if (!typeProp || !t.isStringLiteral(typeProp.value, { value: "skill_listing" })) {
        return false;
    }
    return (
...
function isSkillListingRenderCase(path: NodePath<t.SwitchCase>): boolean {
    if (!t.isStringLiteral(path.node.test, { value: "skill_listing" })) return false;
...
function getSkillListingRenderRootCall(
    ...
    const returnStmt = getSkillListingRenderStatements(path).find((stmt): stmt is t.ReturnStatement =>
...
function patchSkillListingAttachment(path: NodePath<t.ObjectExpression>): boolean {
    ...
    const insertIndex = path.node.properties.findIndex(
        (prop) => t.isObjectProperty(prop) && getObjectKeyName(prop.key) === "skillCount",
    );
                ...
                Program: {
                    exit(path) {
                        ...
                        if (renderCandidates.length === 1) {
                            ...
                            patchedRenderer = patchSkillListingRenderer(
                                renderCandidates[0],
                            );
                        }
                    },
    ...
    verify: (code, ast) => {
        ...
        traverse.default(verifyAst, {
            ...
            ObjectExpression(path) {
                ...
                const skillNamesProp = getObjectPropertyByName(
                    path.node,
                    "skillNames",
                );
