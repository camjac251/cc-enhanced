...
test("skill-listing-ui adds skillNames metadata and a visible summary", async () => {
    ...
    assert.equal(
        output.includes("function _claudePatchFormatSkillListingSummary(attachment)"),
        true,
    );
    assert.equal(
...
test("skill-listing-ui verify fails when skillNames metadata is removed", async () => {
    ...
    assert.equal(typeof result, "string");
    assert.equal(
        String(result).includes("skill_listing attachment is missing skillNames metadata"),
        true,
    );
});
