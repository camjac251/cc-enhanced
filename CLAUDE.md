...
| All 32 patches | `src/patches/<tag>.ts` (each ships `<tag>.test.ts`) |
...
| UX | Terminal interface polish. `plan-diff-ui`, `plan-compact-execute`, `no-collapse`, `subagent-model-tag`, `skill-listing-ui` |
...
6. **When the total patch count changes** (adding or removing a patch), update every place the count appears, in the same change:
   - `CLAUDE.md` intro (`applies 32 verifiable patches`).
