...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Tested-Claude_Code_2.1.170-8A2BE2.svg" alt="Tested against Claude Code 2.1.170">
</p>
...
| [`skill-activation-notice`](src/patches/skill-activation-notice.ts) | When a `paths`/`global-paths` skill activates because a touched file matched, a notice surfaces the activated skill name and file instead of the activation being silent. |
...
Current target: **Claude Code 2.1.170**. Tracks the latest upstream release and is updated with each upstream bump. Older versions are not maintained or tested; when upstream breaks a patch, it is fixed forward rather than kept backward-compatible. Run `claude --version` on the promoted binary to confirm the active target.
