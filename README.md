...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Tested-Claude_Code_2.1.221-8A2BE2.svg" alt="Tested against Claude Code 2.1.221">
</p>
...
Current target: **Claude Code 2.1.221**. Tracks the latest upstream release and is updated with each upstream bump. Older versions are not maintained or tested; when upstream breaks a patch, it is fixed forward rather than kept backward-compatible. The immediately previous clean bundle may be retained for release-diff evidence, but it is not a matcher, fixture, matrix, or promotion target. Run `claude --version` on the promoted binary to confirm the active target.
