...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Tested-Claude_Code_2.1.132-8A2BE2.svg" alt="Tested against Claude Code 2.1.132">
</p>
...
claude --version
# 2.1.132 (Claude Code; patched: shell-quote-fix, bash-prompt, ..., signature)
...
flowchart LR
    ...
    Current --> NewBuild[["build N<br/>patched 2.1.132"]]
...
mise run native:update -- <version>               # Pin a specific version (e.g. 2.1.132)
...
Current target: **Claude Code 2.1.132**. Tracks the latest upstream release and is updated with each upstream bump. Older versions are not maintained or tested; when upstream breaks a patch, it is fixed forward rather than kept backward-compatible. Run `claude --version` on the promoted binary to confirm the active target.
