...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Tested-Claude_Code_2.1.157-8A2BE2.svg" alt="Tested against Claude Code 2.1.157">
</p>
...
| [`effort-stack`](src/patches/effort-stack.ts) | Lets `CLAUDE_CODE_EFFORT_LEVEL=max` win over the upstream resolver short-circuit that otherwise forces `xhigh` whenever `settings.ultracode` is true. Stack max effort with ultracode (workflow orchestration) by setting `CLAUDE_CODE_EFFORT_LEVEL=max` in env and `"ultracode": true` at the top level of `~/.claude/settings.json`. The ultrathink notification still reports the selected tier accurately. |
...
Current target: **Claude Code 2.1.157**. Tracks the latest upstream release and is updated with each upstream bump. Older versions are not maintained or tested; when upstream breaks a patch, it is fixed forward rather than kept backward-compatible. Run `claude --version` on the promoted binary to confirm the active target.
