...
| [`child-network-env`](src/patches/child-network-env.ts) | When the routed launcher records the original proxy and CA variables, ordinary child commands receive that pre-route network environment instead of inheriting the local model bridge. The parent client and nested wrapped client launches retain bridge routing. Launches without the snapshot keep stock environment behavior. |
