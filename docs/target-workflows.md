...
| `tools-off-desktop` | Advertise the reduced core inventory while retaining every `NotebookEdit` registration. | Cached or hardcoded client inventory may show removed tools or hide retained ones. | Tool inventory, prompt surface, and reconnect/resume. |
...
For the original native Desktop app in local mode, the app must actually select and launch the separate patched Code candidate; this repository deliberately does not replace the Desktop-managed artifact or activate that integration automatically. For Remote Control or self-hosted mode, the exact receipt-bound patched CLI runs on the host/runner and the native app remains a client, subject to the compatibility probes above.
